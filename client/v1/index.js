...
console.log('Deals sorted by price (lowest to highest):', dealsSortedByPrice);
...
console.log('Deals sorted by date (recent to old):', dealsSortedByDate);

// 🎯 TODO 6: Filter a specific percentage discount range
// 1. Filter the list of deals between 50% and 75%
// 2. Log the list

const dealsWithDiscountBetween50And75 = deals.filter(deal => deal.discount >= 50 && deal.discount <= 75);
console.log('Deals with discount between 50% and 75%:', dealsWithDiscountBetween50And75);

// 🎯 TODO 7: Average percentage discount
// 1. Determine the average percentage discount of the deals
// 2. Log the average

const dealsWithDiscount = deals.filter(deal => deal.discount !== null && deal.discount !== undefined);
const totalDiscount = dealsWithDiscount.reduce((sum, deal) => sum + deal.discount, 0);
const averageDiscount = totalDiscount / dealsWithDiscount.length;
...
const dealCountByCommunity = Object.keys(communities).reduce((acc, communityName) => {
  acc[communityName] = communities[communityName].length;
  return acc;
}, {});

console.log('Number of deals by community:', dealCountByCommunity);
...
const vintedWithoutDeletedItem = VINTED.filter(item => item.uuid !== 'f2c5377c-84f9-571d-8712-98902dcbb913');
console.log('VINTED list after deletion:', vintedWithoutDeletedItem);
console.log('New list length:', vintedWithoutDeletedItem.length, '(was', VINTED.length, ')');
